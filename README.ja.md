...
<p align="center">
  最新版: <strong>0.2.1</strong>（2026-09-13 公開）
</p>
<!-- END:latest-release -->
...
設置されるのは署名・公証済みの同じ `Reborn.app` で、置き場所も `/アプリケーション` です。
初回起動・権限・アプリ内アップデートの挙動は手動インストールと何も変わりません。
コマンドが 2 つなのは、このリポジトリ自身が tap を兼ねており、`brew tap` に場所を教える必要があるためです。
...
Homebrew で入れた場合、Reborn は自分自身を更新できるため `brew upgrade` はこのアプリを対象外にします
（そうしないと、更新したばかりの版を brew が上書きしてしまいます）。アプリ内アップデータを使うか、
brew から明示的に更新するなら `brew upgrade --cask --greedy reborn` です。
...
Homebrew で入れた場合も、**先にアプリ内アンインストール**を実行してください。ログイン項目の登録は
macOS に API が無く `brew uninstall` では解除できません。そのうえで
`brew uninstall --cask --zap reborn` を実行すると brew 側の記録も消えます。
...
- **Mac App Store では配布できません**（App Sandbox 非対応のため）。配布は DMG のみで、
  ここから直接ダウンロードするか Homebrew 経由で入れる形になります。
